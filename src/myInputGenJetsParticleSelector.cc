...
#include "BristolAnalysis/NTupleTools/interface/myInputGenJetsParticleSelector.h"
...
void myInputGenJetsParticleSelector::setIgnoredParticles(const std::vector<unsigned int> &particleIDs)
{
  ignoreParticleIDs = particleIDs;
  std::sort(ignoreParticleIDs.begin(), ignoreParticleIDs.end());
}

void myInputGenJetsParticleSelector::setExcludeFromResonancePids(const std::vector<unsigned int> &particleIDs)
{
  excludeFromResonancePids = particleIDs;
  std::sort( excludeFromResonancePids.begin(), excludeFromResonancePids.end());
}

bool myInputGenJetsParticleSelector::isParton(int pdgId) const{
...
bool myInputGenJetsParticleSelector::isHadron(int pdgId)
{
  pdgId = (pdgId > 0 ? pdgId : -pdgId) % 10000;
...
bool myInputGenJetsParticleSelector::isResonance(int pdgId)
{
...
bool myInputGenJetsParticleSelector::isIgnored(int pdgId) const
{
...
bool myInputGenJetsParticleSelector::isExcludedFromResonance(int pdgId) const
{
  pdgId = pdgId > 0 ? pdgId : -pdgId;
  std::vector<unsigned int>::const_iterator pos =
...
myInputGenJetsParticleSelector::ResonanceState
myInputGenJetsParticleSelector::fromResonance(ParticleBitmap &invalid,
...
bool myInputGenJetsParticleSelector::hasPartonChildren(ParticleBitmap &invalid,
						     const ParticleVector &p,
						     const reco::GenParticle *particle) const {
