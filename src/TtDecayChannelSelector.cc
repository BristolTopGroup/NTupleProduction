...
static const std::string kMCATNLO_GenParticles = "MC@NLO";
...
TtDecayChannelSelector::TtDecayChannelSelector(const edm::ParameterSet& cfg) :
		...
		allowElectron_(false), allowMuon_(false), //
		allow1Prong_(false), //
		allow3Prong_(false) {
	...
	if (restrictTauDecays_) {
		...
		allow3Prong_ = (
				allowedTauDecays.existsAs<bool>("threeProng") ? allowedTauDecays.getParameter<bool>("threeProng") :
						false);
	}
		...
		if (search(top, TopDecayID::tID, inputType)) {
			...
			for (reco::GenParticle::const_iterator td = top->begin(); td != top->end(); ++td) {
				...
				if (search(td, TopDecayID::WID, inputType)) {
					...
					//this is a potential bug if the W radiates another boson. Should be done in a recursive function.
					reco::GenParticle::const_iterator wd =
							inputType == kMCATNLO_GenParticles ? td->begin()->begin()->begin() : td->begin();
					reco::GenParticle::const_iterator wd_end =
							inputType == kMCATNLO_GenParticles ? td->begin()->begin()->end() : td->end();
					for (; wd != wd_end; ++wd) {
	...
	if ((iTop == 2) && (iBeauty == 2)) {
		if (channel == iLep) {
			...
			if (channel == 1) {
				...
				accept = (iElec && allowedDecays_[Elec]) || (iMuon && allowedDecays_[Muon])
...
bool TtDecayChannelSelector::search(reco::GenParticleCollection::const_iterator& part, int pdgId,
	...
	} else if (inputType == kMCATNLO_GenParticles) {
		bool containts_itself(false);
		for (reco::GenParticle::const_iterator daughter = part->begin(); daughter != part->end(); ++daughter) {
			if (daughter->pdgId() == part->pdgId())
				containts_itself = true;
		}
		//the correct top quark is status 2 and has at least 2 daughters (none should be a top quark, i.e. gluon splitting)
		return (std::abs(part->pdgId()) == pdgId && part->status() == TopDecayID::stable && !containts_itself) ? true :
				false;
	} else {
...
bool TtDecayChannelSelector::search(reco::GenParticle::const_iterator& part, int pdgId, std::string& inputType) const {
	if (inputType == kGenParticles) {
		...
		if (std::abs(pdgId) == 24) //the correct W boson is status 2 and has 1 daughter (a W boson with status 3)
			return (std::abs(part->pdgId()) == pdgId && part->status() == TopDecayID::stable) ? true : false;
		else
			return (std::abs(part->pdgId()) == pdgId && part->status() == TopDecayID::unfrag) ? true : false;
	} else {
		return (std::abs(part->pdgId()) == pdgId) ? true : false;
