...
#include "Leptoquarks/BristolNTuple/interface/PatUtilities.h"
...
void BristolNTuple_Taus::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    if (taus.isValid()) {
        ...
        for (; it != it_end; ++it) {
            ...
            if (it->tauID("againstElectron") > 0.5) {
                againstelectrondiscr -> push_back(1);
            }
            if (it->tauID("againstElectron") < 0.5) {
                againstelectrondiscr -> push_back(0);
            }
            if (it->tauID("againstMuon") > 0.5) {
                againstmuondiscr -> push_back(1);
            }
            if (it->tauID("againstMuon") < 0.5) {
                againstmuondiscr -> push_back(0);
            }
            if (it->tauID("byIsolationUsingLeadingPion") > 0.5) {
                byisolationusingleadingpiondiscr -> push_back(1);
            }
            if (it->tauID("byIsolationUsingLeadingPion") < 0.5) {
                byisolationusingleadingpiondiscr -> push_back(0);
            }
            if (it->tauID("leadingPionPtCut") > 0.5) {
                leadingpionptcutdiscr -> push_back(1);
            }
            if (it->tauID("leadingPionPtCut") < 0.5) {
                leadingpionptcutdiscr -> push_back(0);
            }
            if (it->tauID("leadingTrackFinding") > 0.5) {
                leadingtrackfindingdiscr -> push_back(1);
            }
            if (it->tauID("leadingTrackFinding") < 0.5) {
                leadingtrackfindingdiscr -> push_back(0);
            }
            if (it->tauID("leadingTrackPtCut") > 0.5) {
                leadingtrackptcutdiscr -> push_back(1);
            }
            if (it->tauID("leadingTrackPtCut") < 0.5) {
                leadingtrackptcutdiscr -> push_back(0);
            }
            if (it->tauID("trackIsolation") > 0.5) {
                trackisolationdiscr -> push_back(1);
            }
            if (it->tauID("trackIsolation") < 0.5) {
                trackisolationdiscr -> push_back(0);
            }
            if (it->tauID("trackIsolationUsingLeadingPion") > 0.5) {
                trackisolationusingleadingpiondiscr -> push_back(1);
            }
            if (it->tauID("trackIsolationUsingLeadingPion") < 0.5) {
                trackisolationusingleadingpiondiscr -> push_back(0);
            }
            if (it->tauID("ecalIsolation") > 0.5) {
                ecalisolationdiscr -> push_back(1);
            }
            if (it->tauID("ecalIsolation") < 0.5) {
                ecalisolationdiscr -> push_back(0);
            }
            if (it->tauID("ecalIsolationUsingLeadingPion") > 0.5) {
                ecalisolationusingleadingpiondiscr -> push_back(1);
            }
            if (it->tauID("ecalIsolationUsingLeadingPion") < 0.5) {
                ecalisolationusingleadingpiondiscr -> push_back(0);
            }
            if (it->tauID("byIsolation") > 0.5) {
                byisolationdiscr -> push_back(1);
            }
            if (it->tauID("byIsolation") < 0.5) {
