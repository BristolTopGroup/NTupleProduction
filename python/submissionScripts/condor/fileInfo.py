...
def filterFilesByTime(files, filesNewerThan):
    filteredFiles = []
    for currentFile in files:
        if os.path.getmtime(currentFile) > filesNewerThan:
            filteredFiles.append(currentFile)
...
def extractJobnumber(file):
    jobnumber = file.split('_')[-1].split('.')[0]
    number = 0
    try:
